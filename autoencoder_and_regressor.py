...
buddy = experiment_buddy.deploy(
    ...
    wandb_run_name=f"COMP{config.latent_space}b{config.batch_size}",
...
for epoch in range(1, n_epochs):
    ...
    for x_id, (x_batch, y_batch) in enumerate(train_loader):
        ...
        pearson_loss_regressor = cos(y - y.mean(dim=1, keepdim=True), protein_predictions - protein_predictions.mean(dim=1, keepdim=True))
        # run.log({"train/pearson": pearson_loss_regressor, 'step': curr_step_train})
        print(f"train/pearson:{pearson_loss_regressor}")
