...
buddy = experiment_buddy.deploy(
    ...
    # wandb_run_name=f"back2SVR#50latent#MinMaxX#batchNORM+SELU#autoencoder_batch#{config.batch_size}",
...
df_cite_test_inputs = pd.read_hdf(os.path.join(base_dir, "test_cite_inputs.h5"))
...
optimizer = optim.SGD(autoencoder.parameters(), lr=0.01, momentum=0.95)
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        ...
        logging.info(f"val mse epoch {epoch}: {epoch_val_mse.mean()}")
        run.log({"val/mse_epoch": epoch_val_mse.mean(), "epoch": epoch})
        del epoch_val_mse
