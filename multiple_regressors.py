...
buddy = experiment_buddy.deploy(
    ...
    wandb_run_name=f"!!!140_regressors no AE...#COMP{config.latent_space}b{config.batch_size}",
...
multiple_regressors = [Regressor(in_shape=22050, out_shape=1).float().to(device) for _ in range(140)]
...
for epoch in range(1, n_epochs):
    ...
    for x_id, (x_batch, y_batch) in enumerate(train_loader):
        ...
        protein_predictions = list(map(lambda model: model(x), multiple_regressors))
