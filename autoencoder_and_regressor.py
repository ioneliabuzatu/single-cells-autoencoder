...
buddy = experiment_buddy.deploy(
    ...
    wandb_run_name=f"CoupledRegressor#sumMSE#50latent#MinMaxX#batchNORM+SELU#autoencoder_batch#{config.batch_size}",
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        for x_id, (x_batch, y_batch) in enumerate(val_loader):
            ...
            va_preds.append(val_protein_predictions)
            val_mse_regressor = loss_fn_regressor(val_protein_predictions, y_batch)
